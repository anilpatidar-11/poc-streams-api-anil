...
export default class VideoThumbnailController {
  async uploadVideo({ request, response, i18n }: HttpContext) {
    try {
      ...
      if (duration <= 0) {
        return ResponseHelper.badRequest(response, i18n.t('thumbnail.invalid_duration'))
      }
      ...
      return ResponseHelper.success(response, i18n.t('thumbnail.video_processed'), {
      ...
      return ResponseHelper.serverError(response, i18n.t('thumbnail.processing_failed'), {
  ...
  async getThumbnails({ params, response, i18n }: HttpContext) {
    try {
      ...
      try {
        ...
        if (files.length === 0) {
          return ResponseHelper.notFound(response, i18n.t('thumbnail.thumbnails_not_found'))
        }
        ...
        return ResponseHelper.notFound(response, i18n.t('thumbnail.video_folder_not_found'))
      }
    } catch (error: any) {
      return ResponseHelper.serverError(response, i18n.t('thumbnail.internal_error'), {
        details: error.message,
      })
    }
  }
  ...
  async getVideos({ response, i18n }: HttpContext) {
    try {
      ...
      try {
        ...
        return ResponseHelper.success(response, i18n.t('thumbnail.videos_fetched'), {
          count: videoList.length,
          videos: videoList,
        })
      } catch (error) {
        return ResponseHelper.success(response, i18n.t('thumbnail.no_videos_yet'), [])
      }
    } catch (error: any) {
      return ResponseHelper.serverError(response, i18n.t('thumbnail.internal_error'), {
        details: error.message,
  ...
  private extractFrames(videoPath: string, outputDir: string, count: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ffmpeg(videoPath)
        .on('end', () => resolve())
        ...
        .outputOptions([
          '-vf',
          'scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2', // aspect ratio safe
          '-sws_flags',
          'lanczos', // ← yeh sharp banayega
        ])
    })
  }
}
